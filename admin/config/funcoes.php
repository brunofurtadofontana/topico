<?php
	...
	switch ($code) {
		...
		case 28:
			...
			$senhaNova = Rand(1,1000).time();
			$senhaCript = md5($senhaNova);
			$qrEmail = mysqli_query($con,"select usuario_email from usuario where usuario_id = '$id' ")or die(mysqli_error($con));
			$sw = mysqli_fetch_assoc($qrEmail);
			$emailDestinatario = $sw['usuario_email'];
			$qr = mysqli_query($con,"UPDATE usuario SET usuario_senha = '$senhaCript' where usuario_id = '$id' ")or die(mysqli_error($con));
			...
			if(!$qr){
				...
				if( PATH_SEPARATOR ==';'){ $quebra_linha="\r\n";
	                  ...
	                  $mensagemHTML .="<h3>Usuario: $emailDestinatario</h3>";
	                  $mensagemHTML .="<h3>Sua nova senha é: <span style='color:red'>$senhaNova</span></h3>";
